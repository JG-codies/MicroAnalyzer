...
def cell_training(nd2_path, output_path, images_keys=DEFAULT_IMAGES_NAMES, masks_keys=DEFAULT_IMAGE_MASK_NAMES):
    ...
    images, masks = MicroscopyPreprocessor().prep_images_and_masks_for_cell_segmentation(images, masks)
...
def fluo_training(nd2_path, output_path, images_keys=DEFAULT_IMAGES_NAMES, fluo_keys=DEFAULT_FLUO_NAMES,
    ...
    # prepare images for training
    images, masks = MicroscopyPreprocessor().prep_images_and_masks_for_fluo_segmentation(images, fluo, masks)
...
def __parse_args():
    ...
    if args.type in [CELL_SEGMENTATION_TYPE, ALL_SEGMENTATION_TYPE] and not args.image_masks_keys:
        args.image_masks_keys = {f'Threshold ({name})' for name in args.images_keys}

    if args.type in [FLUO_SEGMENTATION_TYPE, ALL_SEGMENTATION_TYPE] and not args.fluo_masks_keys:
